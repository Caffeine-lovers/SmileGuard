        ...
        <View style={{ flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'flex-end' }}>
          <View style={{ flex: 1, backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, overflow: 'hidden' }}>
            <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingVertical: 24, paddingBottom: 32 }}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 18, fontWeight: '700', color: '#111' }}>Appointment Billing History</Text>
                  <Text style={{ fontSize: 12, color: '#666', marginTop: 4 }}>View all bills for this appointment</Text>
                </View>
                <TouchableOpacity
                  onPress={() => {
                    setShowBillingHistoryModal(false);
              ...
              {loadingBillingHistory ? (
                ...
                <View style={{ justifyContent: 'center', alignItems: 'center', paddingVertical: 40 }}>
                  <Text style={{ fontSize: 48, marginBottom: 8 }}>📋</Text>
                  <Text style={{ fontSize: 16, fontWeight: '600', color: '#111', marginBottom: 4 }}>No Billing Records</Text>
                  <Text style={{ fontSize: 13, color: '#666', textAlign: 'center' }}>No billing records found for this appointment.</Text>
                </View>
              ) : (
                <View>
                  <Text style={{ fontSize: 14, fontWeight: '700', color: '#111', marginBottom: 16 }}>Records ({appointmentBillings.length})</Text>
                  {appointmentBillings.map((bill) => (
                    ...
                    >
                      ...
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                        <View style={{ flex: 1 }}>
                          <Text style={{ fontSize: 11, color: '#666', fontWeight: '600' }}>Bill #{bill.id?.slice(0, 8).toUpperCase()}</Text>
                          <Text style={{ fontSize: 12, fontWeight: '600', color: '#111', marginTop: 4 }}>{bill.description || 'Service Charge'}</Text>
                        </View>
                        <View
                          style={{
                            backgroundColor:
                      ...
                      >
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                          ...
                          <Text style={{ fontSize: 14, fontWeight: '700', color: '#111' }}>${bill.amount.toFixed(2)}</Text>
                        </View>
                        {bill.discount_amount && (
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                            <Text style={{ fontSize: 11, color: '#666' }}>Discount ({bill.discount_type})</Text>
                            <Text style={{ fontSize: 14, fontWeight: '700', color: '#ef4444' }}>-${bill.discount_amount.toFixed(2)}</Text>
                          </View>
                        )}
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                          <Text style={{ fontSize: 11, color: '#666', fontWeight: '600' }}>Final</Text>
                          <Text style={{ fontSize: 14, fontWeight: '700', color: '#0b7fab' }}>${(bill.final_amount || bill.amount).toFixed(2)}</Text>
                        </View>
                      </View>
                      ...
                      <View style={{ gap: 6 }}>
                        ...
                        {bill.payment_method && (
                          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text style={{ fontSize: 11, color: '#666' }}>Method</Text>
                            <Text style={{ fontSize: 11, color: '#111', fontWeight: '500', textTransform: 'capitalize' }}>{bill.payment_method}</Text>
                          </View>
                        )}
                      </View>
                    </View>
