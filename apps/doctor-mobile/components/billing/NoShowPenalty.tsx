...
export default function NoShowPenalty({
  billings,
  selectedPatientId,
  ...
  const noShowBillings = billings.filter((b) => {
    ...
    return desc.includes('no-show') || desc.includes('no show');
  });

  // Calculate totals for no-show penalties only
  const calculateTotals = () => {
    let total = 0;
    let paid = 0;
    let pending = 0;

    noShowBillings.forEach((record) => {
      ...
      <View style={{ padding: 16, gap: 16 }}>
        ...
        <View style={{ marginBottom: 8 }}>
          ...
          <Text style={{ fontSize: 13, color: '#666' }}>
            Manage no-show penalty charges
          </Text>
        </View>
        ...
        {/* No-Show Penalties List */}
        <View>
          <Text style={{ fontSize: 14, fontWeight: '700', color: '#111', marginBottom: 12 }}>
            Charges ({noShowBillings.length})
          </Text>

          {noShowBillings.length === 0 ? (
            <View
              style={{
                backgroundColor: '#fff',
                borderRadius: 12,
            ...
            >
              ...
              <Text style={{ fontSize: 13, color: '#666', textAlign: 'center' }}>
                No no-show penalties found for this patient.
              </Text>
            </View>
          ) : (
            <FlatList
