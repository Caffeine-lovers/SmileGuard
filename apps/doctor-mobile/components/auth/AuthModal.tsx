...
export default function AuthModal({
  ...
  const [isOAuthFlow, setIsOAuthFlow] = useState(false); // Track OAuth sign-up flow
   ...
   * 2. Extract tokens from redirect URL
   * 3. Set session with tokens
   * 4. Create an empty doctor profile in DB (for new users)
   * 5. Navigate to profile setup (step 2)
   */
  const handleGoogleOAuthSignUp = async () => {
    try {
      setLoading(true);
      console.log("[GoogleOAuthSignUp] Starting Google OAuth sign-up...");

      // Step 1: Create redirect URI
      const redirectUri = Linking.createURL('oauth-redirect');
      console.log("[GoogleOAuthSignUp] Redirect URI:", redirectUri);
      ...
      console.log("[GoogleOAuthSignUp] Opening browser with OAuth URL");

      // Step 3: Open in-app browser and wait for redirect
      const result = await WebBrowser.openAuthSessionAsync(data.url, redirectUri);

      console.log("[GoogleOAuthSignUp] Browser session result type:", result.type);

      if (result.type === "success") {
        console.log("[GoogleOAuthSignUp] OAuth successful, extracting tokens...");
        ...
        // Step 4: Set session with tokens
        console.log("[GoogleOAuthSignUp] Setting session with tokens...");
        ...
        console.log("[GoogleOAuthSignUp] User authenticated:", user.email);

        // Step 6: Check if doctor profile exists
        ...
        // Step 7: Create empty doctor profile if it doesn't exist (new OAuth user)
        if (!existingProfile) {
          console.log("[GoogleOAuthSignUp] Creating empty doctor profile for new user...");
          const { error: profileError } = await supabase
            .from("doctors")
            .insert({
              user_id: user.id,
              doctor_email: user.email,
              doctor_name: user.user_metadata?.full_name || user.email?.split('@')[0] || "Doctor",
              // Leave other fields empty for user to fill in during profile setup
            });

          if (profileError) throw profileError;
          console.log("[GoogleOAuthSignUp] Doctor profile created");
        }

        // Step 8: Mark as OAuth flow and pre-populate fields
        setIsOAuthFlow(true);
        setFormData((prev) => ({
          ...prev,
          email: user.email || "",
        ...
        // Step 9: Navigate to profile setup (step 2)
        console.log("[GoogleOAuthSignUp] Navigating to profile setup...");
        setStep(2);
      } else {
        console.log("[GoogleOAuthSignUp] OAuth cancelled by user");
        setLoading(false);
      }
    } catch (err) {
      ...
      console.error("[GoogleOAuthSignUp] Error:", errorMessage);
                    ...
                    <>
                      <>
                        ...
                        <TouchableOpacity
                          ...
                          onPress={handleGoogleOAuthSignUp}
                          disabled={loading}
                        >
